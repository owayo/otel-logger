...
- Claude/Codex の累計使用量を `/stats` と `--summary` で表示し、logs と metrics の二重計上を回避
  ...
  - `conversation.id` を持たない Codex metrics の直近 session fallback は同じ `service.name` 内に限定し、TUI と Exec の同時稼働時に別プロセスの provider / reasoning effort を誤って引き継がない
